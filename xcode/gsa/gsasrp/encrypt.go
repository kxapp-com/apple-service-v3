...
func DecryptDataGCM(key []byte, ciphertextWithHeader []byte) ([]byte, error) {
	ciphertextHeader := ciphertextWithHeader[:3]
	ciphertext := ciphertextWithHeader[3:]
	//fmt.Printf("header %v", ciphertextHeader)
	...
	return gcm.Open(nil, nonce, ciphertext, ciphertextHeader)
}
